...
function findMissingMarkdownAttachments(markdown: string, filePath: string, sourceId: string) { const findings: Array<{ code: string; sourceId: string; message: string }> = []; for (const match of markdown.matchAll(/!?(?:\[[^\]]*\])\(([^)]+)\)/g)) { const raw = match[1].trim(); const reference = raw.startsWith('<') && raw.includes('>') ? raw.slice(1, raw.indexOf('>')) : raw; if (!reference || /^(?:https?:|data:|#)/i.test(reference)) continue; if (!fs.existsSync(path.resolve(path.dirname(filePath), reference))) findings.push({ code: 'ATTACHMENT_MISSING', sourceId, message: `Markdown 附件不存在：${reference}` }); } return findings; }
