...
const receipts = reportPaths.flatMap((reportPath) => findReceipts(readJson(reportPath)));
...
for (const caseId of ids) {
  ...
  if (!Array.isArray(receipt.operationReceipts) || !Array.isArray(receipt.assertionReceipts)) {
    throw new Error(`ADDON_RECEIPT_SURFACE_MISSING:${caseId}`);
  }
  if (!receipt.executionContext || receipt.cleanup?.apiZeroResidue !== true
    || receipt.cleanup?.uiZeroResidue !== true || receipt.cleanup?.uiVerificationObserved !== true) {
...
manifest.reportPaths = [...new Set([...manifest.reportPaths, ...reportPaths])];
publishImmutableArtifact({
  outputRoot: root,
  relativePath: manifestPath,
  content: `${JSON.stringify(manifest, null, 2)}\n`,
  reason: 'register-current-addon-005-011-receipts-after-impact-revalidation',
});
console.log(JSON.stringify({ registered: ids, reportPaths }, null, 2));
