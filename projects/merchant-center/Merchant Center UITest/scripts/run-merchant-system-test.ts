...
(async () => {
  ...
  if (mode === 'run') {
    const { runSystemTest, resolveSystemTestCaseIds } = await import('../../../../tap/scripts/run-system-test');
  ...
  const { runSystemTestFlow } = await import('../../../../tap/scripts/run-system-test-flow');
