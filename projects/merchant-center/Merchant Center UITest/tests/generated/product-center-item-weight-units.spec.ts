import { test } from '../../fixtures/product-center-weight-units.fixture';
...
test.describe('标准商品称重单位正式验收',()=>{
  for(const item of contract.cases.filter(row=>selected.includes(row.caseId))) {
    test(item.title,{tag:['@商品'],annotation:[{type:'canonical-case-id',description:item.caseId}]},async({page,standardWeightUnitsAcceptanceFlow:flow},testInfo)=>{
