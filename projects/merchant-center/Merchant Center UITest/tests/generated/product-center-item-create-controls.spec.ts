import { test } from '../../fixtures/product-center-create-controls.fixture';
...
test.describe('标准商品创建页控件默认与展开正式验收',()=>{
  for(const item of contract.cases.filter(row=>selected.includes(row.caseId))) {
    test(item.title,{tag:['@商品'],annotation:[{type:'canonical-case-id',description:item.caseId}]},async({page,standardCreateControlsFlow:flow},testInfo)=>{
      ...
      try { if(!item.sourceReady)throw Error('SOURCE_CONTRACT_MISSING');await flow.execute(item.caseId);if(!flow.stateRestored)throw Error('CREATE_CONTROLS_RESTORATION_EVIDENCE_INCOMPLETE'); }
        ...
        if(failed)await test.step('失败诊断：保留业务观察和状态恢复结果',async()=>{
          ...
          await testInfo.attach('中文诊断',{body:JSON.stringify({conclusion:failure?String(failure):'当前创建页控件行为不满足正式预期，见逐项期望与实际',category:String(failure).includes('BUSINESS_CONTROL_ABSENT')||flow.assertions.some(a=>a.status==='observed-mismatch')?'business-behavior-difference':'automation-or-source-gap',phase:'create-controls',expected:item.assertionIds,actual:flow.assertions,stateRestored:flow.stateRestored,observations:flow.observations}),contentType:'application/json'});
        });
