...
import { test } from '../../fixtures/product-center-addon-price.fixture';
...
test.describe('加料价格正式验收',()=>{
  for(const item of contract.cases.filter(row=>selected.includes(row.caseId))) {
    test(item.title,{tag:['@商品'],annotation:[{type:'canonical-case-id',description:item.caseId}]},async({page,addonPriceFlow:flow},testInfo)=>{
      ...
      let failure:unknown;
      try { if(!item.sourceReady)throw Error('SOURCE_CONTRACT_MISSING');await flow.execute(item.caseId,buildAddonPriceSourceName(item.caseId,Date.now()));if(!flow.stateRestored)throw Error('CREATE_CONTROLS_RESTORATION_EVIDENCE_INCOMPLETE'); }
        ...
        if(failed)await test.step('失败诊断：保留业务观察和状态恢复结果',async()=>{
          ...
          await testInfo.attach('中文诊断',{body:JSON.stringify({conclusion:failure?String(failure):'当前加料价格行为不满足正式预期，见逐项期望与实际',category:String(failure).includes('BUSINESS_CONTROL_ABSENT')||flow.assertions.some(a=>a.status==='observed-mismatch')?'business-behavior-difference':'automation-or-source-gap',phase:'addon-price',expected:item.assertionIds,actual:flow.assertions,stateRestored:flow.stateRestored,observations:flow.observations}),contentType:'application/json'});
        });
        ...
        const receipt={receiptVersion:'4.0.0' as const,caseId:item.caseId,caseFingerprint:item.caseFingerprint,semanticCaseFingerprint:item.semanticCaseFingerprint,
          ...
          cleanup:{apiZeroResidue:flow.cleanupEvidence?.verifiedZero===true&&flow.cleanupEvidence.exactNameCount===0,uiZeroResidue:flow.uiResidue===0,apiIdentityCounts:flow.cleanupEvidence?{[flow.businessName]:flow.cleanupEvidence.exactNameCount}:{},uiIdentityCounts:flow.uiResidue===undefined?{}:{[flow.businessName]:flow.uiResidue},uiVerificationObserved:flow.uiResidue!==undefined,required:true,objects:flow.cleanupEvidence?.objects??[],serverIds:flow.cleanupEvidence?.registeredServerIds??[],reason:'已按登记ID清理并用精确业务名称核验API/UI残留'},
          businessObservation:{saveActions:flow.saveActions,name:flow.businessName,observations:flow.observations},
