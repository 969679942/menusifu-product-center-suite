import { test } from '../../fixtures/product-center-list.fixture';
import contract from '../../contracts/product-center/test-cases/standard-list-acceptance.json';
...
test.describe('商品列表正式来源验收',()=>{
  for(const item of contract.cases.filter(row=>selected.includes(row.caseId))) {
    test(item.title,{tag:['@商品'],annotation:[{type:'canonical-case-id',description:item.caseId}]},async({page,standardListAcceptanceFlow:flow},testInfo)=>{
      ...
      try { if(!item.sourceReady)throw Error('SOURCE_CONTRACT_MISSING');await flow.execute(item.caseId); }
        ...
        const receipt={receiptVersion:'4.0.0' as const,caseId:item.caseId,caseFingerprint:item.caseFingerprint,semanticCaseFingerprint:item.semanticCaseFingerprint,
          ...
          cleanup:{apiZeroResidue:true,uiZeroResidue:flow.stateRestored,apiIdentityCounts:{},uiIdentityCounts:{},uiVerificationObserved:flow.stateRestored,required:false,reason:'无业务数据写入；列与分页恢复另有实际UI核验'},
