import { test } from '../../fixtures/product-center-required-name.fixture';
...
test.describe('标准商品名称必填正式验收',()=>{
  for(const item of contract.cases.filter(row=>selected.includes(row.caseId))) {
    test(item.title,{tag:['@商品'],annotation:[{type:'canonical-case-id',description:item.caseId}]},async({page,standardRequiredNameFlow:flow},testInfo)=>{
      ...
      let failure:unknown;
      try { if(!item.sourceReady)throw Error('SOURCE_CONTRACT_MISSING');await flow.execute(item.caseId);if(!flow.apiZeroResidue||!flow.uiZeroResidue)throw Error('REQUIRED_NAME_CLEANUP_EVIDENCE_INCOMPLETE'); }
        ...
        if(failed)await test.step('失败诊断：保留业务观察和状态恢复结果',async()=>{
          ...
          await testInfo.attach('中文诊断',{body:JSON.stringify({conclusion:failure?String(failure):'当前名称必填行为不满足正式预期，见逐项期望与实际',category:String(failure).includes('BUSINESS_CONTROL_ABSENT')||flow.assertions.some(a=>a.status==='observed-mismatch')?'business-behavior-difference':'automation-or-source-gap',phase:'required-name',expected:item.assertionIds,actual:flow.assertions,stateRestored:flow.stateRestored,observations:flow.observations}),contentType:'application/json'});
        });
        ...
        const receipt={receiptVersion:'4.0.0' as const,caseId:item.caseId,caseFingerprint:item.caseFingerprint,semanticCaseFingerprint:item.semanticCaseFingerprint,
          ...
          cleanup:{apiZeroResidue:flow.apiZeroResidue,uiZeroResidue:flow.uiZeroResidue,apiIdentityCounts:flow.apiZeroResidue?{'STD005-correlated-creations':0}:{},uiIdentityCounts:flow.uiZeroResidue?{'STD005-correlated-creations':0}:{},uiVerificationObserved:flow.stateRestored,required:true,reason:'负向保存后的请求归属、API完整ID集合与UI列表恢复已分别核验；未知不记零',serverIds:flow.createdIds},
