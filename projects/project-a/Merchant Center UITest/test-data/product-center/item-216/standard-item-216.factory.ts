...
import { createStandardItemAuditPng } from './standard-item-image-data';
import {
...
export type { ProductCenterItemCreateRecord } from '../product-center-item-create-data.factory';
export function standardNameFormatIdentity(caseId: string, uniqueIdentity: string): string {
  if (!uniqueIdentity.startsWith('AUTO_AUDIT_') || uniqueIdentity.length > 48 || /\s/.test(uniqueIdentity)) throw Error('NAME_FORMAT_IDENTITY_INVALID');
  if (caseId === 'TC-ITEM-STD-102') return uniqueIdentity.padEnd(49, 'N') + ' ' + 'N'.repeat(50);
  if (caseId === 'TC-ITEM-STD-103') return uniqueIdentity + '_商品名称😀103';
  throw Error('NAME_FORMAT_CASE_UNSUPPORTED');
}
...
export class StandardItem216Factory {
  ...
  async prepare(caseId?: string): Promise<StandardItem216Context> {
    const context = { ...(await this.itemFactory.prepare()), caseId };
    if (caseId === 'TC-ITEM-STD-102' || caseId === 'TC-ITEM-STD-103') {
      const originalIdentity = standardNameFormatIdentity(caseId, context.originalIdentity);
      return { ...context, originalIdentity, price: '10.00', minimumOrderQuantity: '1',
        cleanupIdentityVariants: [...new Set([originalIdentity, originalIdentity.replace('😀', '')])] };
    }
    return context;
  }
