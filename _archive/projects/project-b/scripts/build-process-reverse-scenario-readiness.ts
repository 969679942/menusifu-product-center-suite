...
const catalog = readJson<ReverseScenarioCatalog>(catalogPath, 'PROCESS_REVERSE_SCENARIO_CATALOG_MISSING');
const registry = readJson<{ cases: Array<{ caseId: string; scenarioId: string; status: string; executionEligible: boolean }> }>(registryPath, 'PROCESS_REVERSE_SCENARIO_REGISTRY_MISSING');
...
const scenarios = catalog.scenarios.map((scenario) => {
  ...
  const sourceRefs = registered?.sourceRefs ?? [];
  const contractRefs = registered?.contractRefs ?? [];
  const staticContractReady = Boolean(registered && sourceRefs.length > 0 && contractRefs.length > 0);
  return {
    ...
    staticContractReady,
    runtimeReady: registered?.executionEligible === true,
    ...
    blockers: staticContractReady
      ? ['execution-grant', 'standard-runtime-receipt', 'context-and-cleanup-evidence']
      : ['project-adapter-source', 'public-contract', 'standard-runtime-receipt'],
...
function readJson<T>(filePath: string, code: string): T {
  if (!fs.existsSync(filePath)) {
    const blockedPath = `${outputPath}.blocked.json`;
    fs.mkdirSync(path.dirname(blockedPath), { recursive: true });
    fs.writeFileSync(blockedPath, `${JSON.stringify({ schemaVersion: '1.0.0', status: 'blocked', code, missingInput: path.relative(merchantRoot, filePath), businessExecutionStarted: false, generatedAt: new Date().toISOString() }, null, 2)}\n`);
