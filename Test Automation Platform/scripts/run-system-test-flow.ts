...
function readFlowRunTerminalCaseIds(input: {
  rootDir: string;
  systemId: string;
  runId: string;
}): string[] {
  const ledgerPath = path.join(input.rootDir, 'output/system-test', input.systemId, input.runId, 'evidence-ledger.json');
  if (!fs.existsSync(ledgerPath)) return [];
  try {
    const ledger = readJson<{ cases?: Array<{ caseId?: string }> }>(ledgerPath);
