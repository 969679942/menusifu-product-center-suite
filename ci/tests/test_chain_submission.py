import importlib.util, inspect, json, os, pathlib, tempfile, unittest
...
class ChainSubmission(unittest.TestCase):
    ...
    bundle_json=json.dumps({
        ...
        'repositories':{
            'pcs':{'revision':'1'*40},
            'mc':{'revision':j.read(j.ROOT/'ci/dependency-manifest.json')['repositories']['mc']['revision']},
            'tap':{'revision':j.read(j.ROOT/'ci/dependency-manifest.json')['repositories']['tap']['revision']},
        },
    })
    ...
    def test_chain_loads_runtime_at_contracts_and_pins_dependencies(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.dict(os.environ,{'MC_RUNTIME_ENV_PATH':str(f)}):
                with patch.object(j.subprocess, 'check_output', side_effect=[self.bundle_json, '[]']):
