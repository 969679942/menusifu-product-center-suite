...
function projectBusinessResults(){
 fs.rmSync(businessDir,{recursive:true,force:true});fs.mkdirSync(businessDir,{recursive:true});const selected=new Set(envelope.selectedCaseIds||[]),chosen=new Map();
 for(const source of resultSources())for(const name of fs.readdirSync(source)){const from=path.join(source,name);if(!fs.statSync(from).isFile())continue;if(!name.endsWith('-result.json')){copy(from,path.join(businessDir,name));continue;}const result=JSON.parse(fs.readFileSync(from,'utf8')),caseId=caseIdOf(result);if(!caseId||(selected.size&&!selected.has(caseId)))continue;const score=JSON.stringify(result).length+(result.steps?.length||0)*1000+(result.attachments?.length||0)*1000;const priority=source===rawDir?0:1;const prior=chosen.get(caseId);if(prior&&(prior.priority>priority||(prior.priority===priority&&prior.score>=score)))continue;chosen.set(caseId,{name,source,result,score,priority});}
