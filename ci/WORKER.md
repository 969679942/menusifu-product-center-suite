...
- `analysis.json` 由 TAP 确定性仲裁器生成独立的执行、证据和身份状态。确定性结论写入 `result-review.json`；`ai-review.json` 仅保留为旧 build-watch 兼容镜像。AI 可辅助修复方案，不得决定执行是否完成或产品是否失败。后续验证必须绑定精确提交、请求和范围。
