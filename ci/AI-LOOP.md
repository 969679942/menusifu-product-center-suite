...
传输的 `analyzed` 表示下载与公共合同校验结束。身份、选择集、执行完整性、bundle、标准收据和产品失败资格由 TAP 确定性仲裁器直接裁决；非业务错误生成 `result-review.json` 并自动登记技术整改，不等待 AI。`ai-review.json` 仅作为旧 build-watch 的兼容镜像，不再拥有状态裁决权。基础合同、报告样本、真实业务执行、跨系统公共平台最终验收仍是不同事实。
